...
class TestFunnySplits:
    ...
    def test_splits_recombine_into_one_run(self, indices):
        ...
        paragraphs = _paragraphs(format_document_bytes(data))
...
class TestDisjointedRuns:
    def test_word_broken_in_the_middle(self):
        data = _docx([["wo", "rd"]])
        paragraph = _paragraphs(format_document_bytes(data))[0]
...
class TestSpacePreservation:
    def test_space_attached_to_left_fragment(self):
        ...
        assert _paragraphs(format_document_bytes(data))[0].text == "hello world"
    ...
    def test_double_space_between_words_is_kept(self):
        data = _docx([["a", "  ", "b"]])
        assert _paragraphs(format_document_bytes(data))[0].text == "a  b"
    ...
    def test_no_space_is_invented_between_mid_word_fragments(self):
        ...
        assert _paragraphs(format_document_bytes(data))[0].text == "supercalifragilistic"
    ...
    def test_tab_between_runs_is_preserved(self):
        ...
        assert _paragraphs(format_document_bytes(data))[0].text == "left\tright"
...
class TestStoryCoverage:
    def test_runs_in_a_table_cell_are_merged(self):
        ...
        buffer = io.BytesIO()
        doc.save(buffer)
        out = format_document_bytes(buffer.getvalue())
