# Vendored from https://github.com/KevinCarr42/Doc-Shrinker
# Source: src/doc_shrinker.py @ commit fc93f22cf1fdcbfdef7f191638228fb014438372 (master, 2026-05-01)
import io
import hashlib
import re
import zipfile
...
# Some docs legitimately contain >100 megapixel TIFFs/scans. Disable PIL's
# decompression-bomb refusal since these are trusted local files.
Image.MAX_IMAGE_PIXELS = None


def compress_docx_images(file_path, quality=80, max_width=1000, target_bytes=None, min_compress_width=800, min_byte_size=50_000):
    p = Path(file_path)
    ...
    if target_bytes is not None:
        # (quality, max_width, force_jpeg). Force-JPEG passes give up the line-art
        # heuristic — needed when chart-heavy docs hit a PNG-quality floor.
        attempts = [
    ...
    for attempt_idx, (q, mw, force_jpeg) in enumerate(attempts, start=1):
        ...
        for part in doc.part.package.parts:
            if "media" in part.partname and any(ext in part.partname for ext in [".jpg", ".png", ".jpeg", ".tif", ".tiff"]):
                ...
                # Cropped images (a:srcRect) need extra source pixels so the visible
                # region isn't upscaled by Word at render time. Never resize below that.
...
def _scan_image_min_widths(docx_path):
    # Walk every drawing in document.xml + header*.xml + footer*.xml and compute the
    # minimum source-pixel width each image part needs to render its visible (post-crop)
    # area at its display size without Word upscaling.
...
def _shrink_image_blob(blob, ext, quality, max_width, min_compress_width, min_byte_size, force_jpeg=False):
    ...
    if keep_png:
        ...
        # don't default to black (causes dark halos around plot edges)
...
def _is_line_art(img):
    # Charts, plots, icons have few unique colors; photos blow past 4k easily.
    # getcolors returns None when the image exceeds the threshold.
    sample = img.copy()
...
def _dedupe_media_in_docx(docx_path):
    ...
    # Rewrite every .rels Target that points at a duplicate so it points at the canonical part
    ...
    # Drop Override entries for parts we're about to delete (avoids "missing part" errors in Word)
...
def _normalize_media_extensions(docx_path):
    # Re-encoding can leave bytes whose format no longer matches the part's filename
    # (e.g. JPEG bytes inside `image13.tiff`). Word handles this loosely but downstream
    # tools and Word's "Save as Picture" misbehave. Rename parts so extension matches content.
    z_in = zipfile.ZipFile(docx_path, "r")
    items = {info.filename: z_in.read(info.filename) for info in z_in.infolist()}
    z_in.close()
